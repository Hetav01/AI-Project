...
# Building will control elevators and passengers.
# And also sets all constraints and will operate full environment.
...
class Building(object):
    '''
        ...
        '''
    # Initial Declarations of the Variables    
    ...
    def empty_building(self):
        ...
        for e in self.elevators:
            e.empty()
        self.remain_passengers_num = 0
    # getting the remaining passengers from building
    def get_remain_passengers_in_building(self):
        return sum([len(x) for x in self.floors_information]) 
    # getting the remaining passengers from elevator
    ...
    # Generating random people requests to train and use model
    ...
    # Defining all the elevator actions to be taken in the building
    ...
    # Printing out the display screen for the building and elevator movements
